...
pevent_t * pstate_sub_preprocessor_main_handler(FILE *fd, int ch)
{
	switch(ch)
	{
		...
		case '<':
			...
			return &pevent_data;
		// check for user defined header file
		case '\"':
...
pevent_t * pstate_reserve_keyword_handler(FILE *fd, int ch)
{
	// check if char is alphanumeric (reserved keyword)
	if(isalnum(ch) || ch == '_')
	{
		word[word_idx++] = ch;
		pevent_data.data[event_data_idx++] = ch;
	...
	else
	{
		...
		// compare with look up table of reserved keywords
		// set property as RES_KEYWORD_DATA or RES_KEYWORD_NON_DATA
		if((pevent_data.property = is_reserved_keyword(word)))
		{
